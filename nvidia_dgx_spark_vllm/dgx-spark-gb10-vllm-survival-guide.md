...
### 🔄 Switching to Gemma 4 (NVFP4-Optimized)

Good news: Red Hat provides NVFP4-quantized Gemma 4 models that work beautifully on DGX Spark. The configuration below adapts your proven x86_64 setup for ARM64/Blackwell optimizations.

**Key differences from x86_64 GPU setups:**
- Lower `--gpu-memory-utilization` (0.70 vs 0.85) — UMA needs headroom
- Marlin backend (via env vars) instead of Cutlass — Blackwell-optimized
- No explicit `--quantization compressed-tensors` — vLLM auto-detects NVFP4
- ARM64-specific image: `eugr/vllm-openai:nightly-aarch64`

### 📄 docker-compose.yml for Gemma 4 NVFP4 on DGX Spark

Add this as a second service in your compose file, or replace the Qwen service:
...
```yaml
  vllm-gemma4:
    ...
    environment:
      - HUGGING_FACE_HUB_TOKEN=${HF_TOKEN:-}
      - VLLM_NVFP4_GEMM_BACKEND=marlin      # Blackwell-optimized backend
      - VLLM_USE_FLASHINFER_MOE_FP4=0
    ...
    volumes:
      ...
      - ./logs:/var/log/vllm
    ports:
      - "8001:8000"                         # Use different port if running alongside Qwen
    command: >
      vllm serve RedHatAI/gemma-4-26B-A4B-it-NVFP4
      --served-model-name gemma-4-26b-it-nvfp4
      --trust-remote-code
      --host 0.0.0.0
      --port 8000
      --gpu-memory-utilization 0.70
...
huggingface-cli download RedHatAI/gemma-4-26B-A4B-it-NVFP4 \
  --local-dir ~/.cache/huggingface/hub/models--RedHatAI--gemma-4-26B-A4B-it-NVFP4
```

### ✅ Verification for Gemma 4
Run the same log check:
```bash
docker compose logs vllm-gemma4 | grep -E "sm_121|marlin|nvfp4"
```
...
**Performance expectation:** 45-55 tokens/sec for Gemma 4 26B NVFP4 on DGX Spark — identical to Qwen 3.6!

> 💡 **Note for x86_64 GPU users**: Your original config with `--quantization compressed-tensors` and `--moe-backend cutlass` works perfectly on RTX 5090/4090. The ARM64/Blackwell optimizations above are specific to DGX Spark's UMA architecture and SM121 tensor cores.
...
| Gemma 4 26B (NVFP4) | MoE, NVFP4-quantized | 45-55 tok/s |
...
**Final Verdict:** (Pending Hardware Validation).
